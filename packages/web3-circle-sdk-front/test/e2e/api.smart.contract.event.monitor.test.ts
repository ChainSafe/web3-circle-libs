import type { SmartContractEventMonitorApi } from '../../src';
import { BLOCKCHAIN } from '../../src';

import { ETH_SEPOLIA_USDC_CONTRACT_ADDRESS, signInAndGetSdk } from './fixtures/fixtures';
...
describe.skip('SmartContractEventMonitorApi Tests', () => {
  let eventMonitorApi: SmartContractEventMonitorApi;
  beforeAll(async () => {
    const sdk = await signInAndGetSdk();
    eventMonitorApi = sdk.smartContractEventMonitor;
  });
  it('Get Event Monitors', async () => {
    const params = {
      blockchain: BLOCKCHAIN.ETH_SEPOLIA,
    };
    const eventMonitors = await eventMonitorApi.get(params);
  ...
  it.only('Create Event Monitor', async () => {
    ...
    const createdMonitors = await eventMonitorApi.create(params);
  ...
  it('Update Event Monitor', async () => {
    ...
    const updatedMonitors = await eventMonitorApi.update(params);
  ...
  it('Delete Event Monitor', async () => {
    ...
    await expect(eventMonitorApi.delete(params)).resolves.toBeUndefined();
  });
