import type { MonitoredTokensApi } from '../../src';
import { BLOCKCHAIN, MONITORED_TOKENS_SCOPE } from '../../src';

import { ETH_SEPOLIA_EURC_TOKEN_ID, signInAndGetSdk, wait } from './fixtures/fixtures';

describe.skip('Monitored tokens Api', () => {
  let monitoredTokensApi: MonitoredTokensApi;
  beforeAll(async () => {
    const sdk = await signInAndGetSdk();
    monitoredTokensApi = sdk.monitoredTokens;
  });
  it('Set', async () => {
